...
"Фотореалистичное изображение {ключевая деталь новости}, в стиле {атмосфера новости}, фон {подходящее окружение}, {дополнительные детали}, кинематографическое освещение, высокая детализация."
""",
...
📷 Промпт для изображения:
"Красочное и стильное изображение {ключевой объект новости}, акцент на {деталь}, современный инстаграммный стиль, мягкое освещение, яркие цвета, идеально подходит для соцсетей."
""",
...
📷 Промпт для изображения:
"Фотореалистичное изображение {главный объект новости}, естественное освещение, сочные цвета, композиция с акцентом на {ключевой элемент}, социальный контекст, приятный глазу фон."
""",
...
📷 Промпт для изображения:
"Яркое, динамичное изображение {ключевой объект}, акцент на {главную эмоцию}, трендовый стиль, неоновая подсветка, современный молодежный дизайн, захватывающий ракурс."
""",
...
📷 Промпт для изображения:
"Минималистичное, но выразительное изображение {ключевой объект}, акцент на {деталь}, контрастные цвета, легкость в восприятии, современный новостной стиль."
""",
...
📷 Промпт для изображения:
"Эстетичное изображение {ключевой объект}, нежные цвета, стиль lifestyle, идеальная композиция, вдохновляющая атмосфера, трендовый визуал, аккуратные детали."
""",
...
📷 Промпт для изображения:
"Яркая, броская обложка YouTube, {ключевой объект}, крупный шрифт с интригующим заголовком, контрастные цвета, динамичный фон, профессиональная графика."
"""
}
...
def generate_social_media_post(news_text, platform, output_language):
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            max_tokens=700,  # Увеличиваем, чтобы хватило на текст и промпт
            temperature=0.7,
        )
        full_response = response.choices[0].message.content.strip()

        # Извлекаем промпт для изображения с помощью регулярного выражения
        match = re.search(r'📷 Промпт для изображения:\s*"(.*?)"', full_response, re.DOTALL)
        if match:
            image_prompt = match.group(1).strip()
             # Заменяем плейсхолдеры (простейший вариант)
            image_prompt = image_prompt.format(
              ключевая_деталь_новости="деталь из новости",
              атмосфера_новости="атмосфера",
              подходящее_окружение="окружение",
              дополнительные_детали="детали",
              ключевой_объект_новости="объект",
              деталь="деталь",
              главный_объект_новости = "объект",
              ключевой_элемент = "элемент",
              главную_эмоцию = "эмоция",
              ключевой_объект = "объект"
            )
        else:
            image_prompt = None

        # Текст поста - это все, что до "🔹 Теперь сгенерируй" (или весь ответ, если этой фразы нет)
        ...
        for platform in platforms:
            ...
            if result["image_prompt"]:
                ...
                if image_result["error"]:
                    ...
                    generated_texts[platform]["image_url"] = image_result["url"] #Сохраняем URL в результат
            else:
