...
"Фотореалистичное изображение {ключевая_деталь_новости}, в стиле {атмосфера_новости}, фон {подходящее_окружение}, {дополнительные_детали}, кинематографическое освещение, высокая детализация."
""",
...
📷 Промпт для изображения:
"Красочное и стильное изображение {ключевой_объект_новости}, акцент на {деталь}, современный инстаграммный стиль, мягкое освещение, яркие цвета, идеально подходит для соцсетей."
""",
...
📷 Промпт для изображения:
"Фотореалистичное изображение {главный_объект_новости}, естественное освещение, сочные цвета, композиция с акцентом на {ключевой_элемент}, социальный контекст, приятный глазу фон."
""",
...
📷 Промпт для изображения:
"Яркое, динамичное изображение {ключевой_объект_новости}, акцент на {главную_эмоцию}, трендовый стиль, неоновая подсветка, современный молодежный дизайн, захватывающий ракурс."
""",
...
📷 Промпт для изображения:
"Минималистичное, но выразительное изображение {ключевой_объект_новости}, акцент на {деталь}, контрастные цвета, легкость в восприятии, современный новостной стиль."
""",
...
📷 Промпт для изображения:
"Эстетичное изображение {ключевой_объект_новости}, нежные цвета, стиль lifestyle, идеальная композиция, вдохновляющая атмосфера, трендовый визуал, аккуратные детали."
""",
...
📷 Промпт для изображения:
"Яркая, броская обложка YouTube, {ключевой_объект_новости}, крупный шрифт с интригующим заголовком, контрастные цвета, динамичный фон, профессиональная графика."
"""
}
...
def generate_social_media_post(news_text, platform, output_language):
    ...
    try:
        ...
        # Текст поста
        text_match = re.search(r'(.*?)(🔹 Теперь сгенерируй|📷 Промпт для изображения:)', full_response, re.DOTALL) #Исправлено
        if text_match:
    ...
    if request.method == 'POST':
        ...
        for platform in platforms:
            ...
            if result["image_prompt"]:
                ...
                generated_texts[platform]["image_url"] = None  # Добавлено
